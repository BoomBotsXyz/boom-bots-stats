...
// returns a promise representing the request
async function s3CopyObjectPromise(params) {
  return new Promise((resolve,reject) => {
    try {
      S3.copyObject(params, (err,data) => {
        if(err) {
          var params2 = { Bucket: params.Bucket, CopySource: params.CopySource, Key: params.Key }
          err.stack = `Could not S3 copy ${JSON.stringify(params2)}\n${err.stack}`
          reject(err)
        } else resolve(data)
      })
    } catch(e) {
...
exports.s3CopyObjectPromise = s3CopyObjectPromise
...
async function snsPublishMessage(msg) {
  var params = {
    Message: msg,
    TopicArn: "arn:aws:sns:us-west-2:776862011846:BoombotsStatsSnsTopic"
  }
...
async function snsPublishError(event, err) {
  ...
  var msg = `The following error occurred in the boombots-stats api${eventString} :\n${errMsg}`
